...
def cells() -> list[dict]:
    ...
    out.append(code(
        "import json\n"
        "\n"
        "# The ABSOLUTE per-seed curve lived only in outputs/p6ladder/search, destroyed 2026-07-27\n"
        "# (absent from git AND from the backup). Load it when present; otherwise F11 falls back to\n"
        "# the paired-ascent exhibit, which the git-tracked verdict artifact fully supports.\n"
        "grid = None\n"
        "try:\n"
        "    from apply_bstar_rule import load_grid\n"
        "    grid = load_grid()   # {winner: {budget: {seed: val_DSR}}}\n"
        ...
        "except BaseException as exc:   # load_grid raises SystemExit on an incomplete rung\n"
        "    grid = None\n"
        "    print(f'absolute curve archive unavailable ({exc}) -> F11 uses the ascent fallback')\n"
        "\n"
    ...
    out.append(code(
        ...
        "\n"
        "fig = (F.budget_curve_exhibit(grid, b_star=400_000) if grid is not None\n"
        "       else F.budget_ascent_exhibit(verdict, b_star=400_000))  # F11, R77 mandatory\n"
        "fig"
    ))
    out.append(md(
        "### Reading the curve\n"
    ...
    out.append(md(
        ...
        "will carry — the M1/M1b run-diagnostics capture "
